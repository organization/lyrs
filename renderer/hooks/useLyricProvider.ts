...
export type LyricProviderAccessor = Accessor<RendererLyricProvider> &
  LyricProviderThis;
...
export const useLyricProvider = (): LyricProviderAccessor => {
  ...
  const providerName = () =>
    config()?.lyricProvider ?? DEFAULT_CONFIG.lyricProvider;
  ...
  result.list = async () =>
    window.ipcRenderer.invoke('lyric-provider:get-all-lyric-providers');
